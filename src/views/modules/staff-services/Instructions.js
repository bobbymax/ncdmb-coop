...
export const Instructions = (props) => {
  ...
  const instructions = useSelector(
    (state) => state.payments.instructions.collection
  );
        ...
        <Row>
          <Col>
            <Table striped hover>
              ...
              <tbody>
                {props.instructions.collection.length !== 0
                  ? props.instructions.collection.map((instruction) => {
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    update: (entity, id, body, broadcast) =>
      dispatch(update(entity, id, body, broadcast)),
